...
///
/// `None` означает «значение выходит за собственный корень» — то есть в нём
/// больше `..`, чем компонентов (`C:/work/../../Windows`). Это осознанно
...
/// Отделяет корень от остатка.
fn split_root(unified: &str) -> (String, &str) {
    // URL: scheme://authority. Authority кончается на первом /, ? или #.
    if let Some(scheme_end) = unified.find("://") {
        let scheme = &unified[..scheme_end];
        if !scheme.is_empty()
            && scheme
                .chars()
                .all(|c| c.is_ascii_alphanumeric() || c == '+' || c == '-' || c == '.')
        {
            let after = &unified[scheme_end + 3..];
...
/// Является ли символ границей компонента. `?` и `#` — для URL: правило на
/// `https://host` обязано покрывать `https://host?query`.
