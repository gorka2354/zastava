...
pub struct CallRecord {
    ...
    /// Полные аргументы вызова — ТОЛЬКО при `log.log_args = true`, и всегда
    /// пропущенные через [`crate::mask`]: значения под секретными ключами,
    /// значения вида токена и учётные данные в URL заменяются на `<masked>`,
    /// слишком длинные — усекаются. Маскировка эвристическая: она снимает
    /// типовой риск, но не превращает журнал в место для секретов.
